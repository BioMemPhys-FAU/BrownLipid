...
def test_lennard_kernel():
    ...
    #Parameters
    lj_sig = np.ones( (pos.shape[0], pos.shape[0]) ) * 0.7706
    lj_eps = np.ones( (pos.shape[0], pos.shape[0]) ) * 0.3221
    ...
    inner_rij, inner_rij_sq, mask, outer_rij, outer_rij_sq = force.filter_pairlist(cutoff = vdw_cutoff**2, rij = rij, rij_sq = rij_sq)

    force_true  = naive_force( r = rij_norm , sig = lj_sig[0,0], eps = lj_eps[0,0]).reshape(-1,1) * rij / rij_norm.reshape(-1, 1)
    ...
    energy_true = naive_energy(r = rij_norm , sig = lj_sig[0,0], eps = lj_eps[0,0]).astype(np.float32)
    virial_true = naive_virial(r = rij_norm , sig = lj_sig[0,0], eps = lj_eps[0,0]).astype(np.float32)
    ...
    plt.plot(r, naive_force(r, lj_sig[0,0], lj_eps[0,0]), color = 'green')
    plt.ylim(-2, 2)
    plt.xlim(0, 2)
    plt.legend(loc = 'upper right')
    plt.savefig("test_force_kernel.png", dpi = 300)
    ...
    plt.plot(r, naive_energy(r, lj_sig[0,0], lj_eps[0,0]), color = 'green')
    plt.ylim(-2, 2)
    plt.xlim(0, 2)
    plt.legend(loc = 'upper right')
    plt.savefig("test_energy_kernel.png", dpi = 300)
    ...
    plt.plot(r, naive_virial(r, lj_sig[0,0], lj_eps[0,0]), color = 'green')
    plt.ylim(-2, 2)
    plt.xlim(0, 2)
    plt.legend(loc = 'upper right')
    plt.savefig("test_virial_kernel.png", dpi = 300)
...
def test_lennard_kernel_different():
    ...
    lj_sig[4:, 4:] = (0.7706 + 0.88 ) / 2
    lj_sig[:, 4:] = (0.7706 + 0.88 ) / 2
    lj_sig[4:, :] = (0.7706 + 0.88 ) / 2
    
    lj_eps[4:, 4:] = (0.3221 * 0.88 )**0.5
    lj_eps[:, 4:] = (0.3221 * 0.88 )**0.5
    lj_eps[4:, :] = (0.3221 * 0.88 )**0.5
    ...
    force_true  = naive_force( r = rij_norm , sig = lj_sig[ pairlist_true[:, 0], pairlist_true[:, 1] ], eps = lj_eps[ pairlist_true[:, 0], pairlist_true[:, 1] ]).reshape(-1,1) * rij / rij_norm.reshape(-1, 1)
    ...
    energy_true = naive_energy(r = rij_norm , sig = lj_sig[pairlist_true[:, 0], pairlist_true[:, 1]], eps = lj_eps[ pairlist_true[:, 0], pairlist_true[:, 1] ]).astype(np.float32)
    virial_true = naive_virial(r = rij_norm , sig = lj_sig[pairlist_true[:, 0], pairlist_true[:, 1]], eps = lj_eps[ pairlist_true[:, 0], pairlist_true[:, 1] ]).astype(np.float32)
    ...
    plt.scatter(rij_norm, (force_true/rij*rij_norm.reshape(-1, 1))[:, 1],         s=10, marker = "x", lw=3, color = 'r', label = 'Naive')
    plt.plot(r, naive_force(r, lj_sig[0,0], lj_eps[0,0]), color = 'green')
    plt.plot(r, naive_force(r, lj_sig[-1,-1], lj_eps[-1,-1]), color = 'green')
    plt.ylim(-2, 2)
    plt.xlim(0, 2)
    plt.legend(loc = 'upper right')
    plt.savefig("test_force_kernel.png", dpi = 300)
    ...
    plt.scatter(rij_norm     , energy_true, s=10, marker = "x", lw=3, color = 'r', label = 'Naive')
    plt.plot(r, naive_energy(r, lj_sig[0,0], lj_eps[0,0]), color = 'green')
    plt.plot(r, naive_energy(r, lj_sig[-1, -1], lj_eps[-1, -1]), color = 'green')
    plt.ylim(-2, 2)
    plt.xlim(0, 2)
    plt.legend(loc = 'upper right')
    plt.savefig("test_energy_kernel.png", dpi = 300)
    ...
    plt.scatter(rij_norm,      virial_true, s=10, marker = "x", lw=3, color = 'r', label = 'Naive')
    plt.plot(r, naive_virial(r, lj_sig[0,0], lj_eps[0,0]), color = 'green')
    plt.plot(r, naive_virial(r, lj_sig[-1, -1], lj_eps[-1, -1]), color = 'green')
    plt.ylim(-2, 2)
    plt.xlim(0, 2)
    plt.legend(loc = 'upper right')
    plt.savefig("test_virial_kernel.png", dpi = 300)
...
@pytest.mark.parametrize("L, r, apl, equalize", [(20, 6.18, 0.66, True), (20, 4.37, 0.66, True), (20, 6.18, 0.66, False), (20, 4.37, 0.66, False)])
...
def test_lj_with_domains(L):
    ...
    uni = brownlipid.Universe(
                 ...
                 hard_boundaries = {"Circle":[[np.inf, r, L/2, L/2]]},
                        softwall = True,
                 external_forces = {'epsilon': 0.3221, 'sigma': 0.7706, 'r_vdw': 5, 'r_list':100.0, 'nstlist':1, 'epsilon_domains':0.88, 'sigma_domains':0.7706},
                         output  = f"access_domain/trajectory_simple_test",
                         nstchk  = int(nsteps) )
